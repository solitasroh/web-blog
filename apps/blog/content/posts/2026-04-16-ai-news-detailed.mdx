...
1. **세션 1**: 메인 서비스 분석 및 API 계약 재정의
2. **세션 2 (사이드 챗)**: 데이터베이스 스키마 변경 영향도 확인
3. **세션 3**: 새로운 서비스 구현
4. **세션 4**: 통합 테스트 작성
...
**신청 URL**: [openai.com/cyber-defense](https://openai.com/cyber-defense)

### 실제 활용 사례

1. **취약점 스캐닝**: 대규모 코드베이스 자동 분석
2. **침해 대응**: 로그 분석 및 공격 경로 추적
3. **보안 테스트**: 자동화된 펜테스트 시나리오 생성
...
1. **장문 문서 생성**: 100K+ 토큰 컨텍스트
2. **대화형 AI**: 다중 턴 대화 유지
3. **코드 생성**: 대규모 코드베이스 이해
